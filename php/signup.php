        ...
        if (filter_var($email, FILTER_VALIDATE_EMAIL)) { // if email is valid
            ...
            if (mysqli_num_rows($sql) > 0) { // if email already exist
                ...
                if (isset($_FILES['image'])) { // if file is uploaded
                    ...
                    $extensions = ['png', 'jpeg', 'jpg']; // these are some valid img ext and we've stored them in array
                    if (in_array($img_ext, $extensions) === true) { // if user uploaded img ext is matched with any array extension
                        $time = time(); // this will return us current time...
                                        // we need this time because when you're uploading user img to in our folder we rename user file with current time
